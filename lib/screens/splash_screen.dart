            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Container(width: 110, height: 110,
                decoration: BoxDecoration(color: AppTheme.primary.withOpacity(0.15),
                    shape: BoxShape.circle, border: Border.all(color: AppTheme.primary, width: 2)),
                child: const Icon(Icons.wifi_tethering_rounded, size: 60, color: AppTheme.primary)),
              const SizedBox(height: 20),
              const Text('netcode', style: TextStyle(fontSize: 36,
                  fontWeight: FontWeight.w900, color: AppTheme.primary, letterSpacing: 2)),
              const SizedBox(height: 8),
              const Text('Rede local offline  Mesh P2P',
                  style: TextStyle(fontSize: 13, color: AppTheme.textSecondary, letterSpacing: 1)),
            ]))),
      )),
    );
  }
